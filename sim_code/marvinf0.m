...
post_durations = zeros(200,1);
avg_up = zeros(200,1);
avg_stimuli = zeros(200,1);
...
parfor i = 1:300
    ...
    [window,window_t] = OUNoiseWindow(theta,0.5,window_duration,dt,save_dt,numsignals,noise_up(start+1),i);
    ...
    post_durations(i,1) = post_nostimuli*dt;
    avg_stimuli(i,1) = mean(complete_window(start/dt:start/dt+duration_s/dt,1));
    avg_up(i,1) = mean(complete_window(start/dt:start/dt+post_nostimuli,1));
